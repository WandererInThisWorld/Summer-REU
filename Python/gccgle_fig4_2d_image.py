...
for n in range(1, int(nmax) + 1):
    V[0][0]
    np.mean(ifft2(V))
    Nv = fft2((1 - 1j*omega) * ifft2(V) - (1 + 1j*alpha)*ifft2(V)*abs(ifft2(V))**2 - g * mu * np.exp(1j*chi) * V[0][0]/(length**2)) # mess around with this, repolacing [][] with np.mean(ifft2)
    a = E2*V + Q*Nv
